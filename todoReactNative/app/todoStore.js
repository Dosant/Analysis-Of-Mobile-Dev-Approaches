...
function todos(state = defaultState, action) {
    ...
    case 'UNDONE_TODO':
        ...
        const allTodosContainingPending = state.todos
            .map((todo) => {
                return todo === action.todo ? pendingTodo : todo;
            });

        return Object.assign({}, state, {
            todos: allTodosContainingPending,
            filteredTodos: getFilteredTodos(allTodosContainingPending, state.filter),
        });
